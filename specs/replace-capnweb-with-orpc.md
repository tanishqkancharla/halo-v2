...
Assumption: oRPC lazy routers are for code splitting (`() => import(...)`) and do not receive request context. Plugin RPCs are dynamic, so Halo does not use lazy for them. Host procedures are a module-level `router` of `os.<procedure>.handler(...)` that read `HaloContext` from `upgrade`. `plugins` on that object is a handler that calls `context.plugins.route`. oRPC's matcher only indexes `/plugins`, so `HaloRPCHandler` resolves `/plugins/{id}/...` to that handler and passes the full path. `PluginService.route` looks up the mounted procedure with `getRouter` / `call`. Do not wrap the host object in `os.router()`; that hides extra keys behind the contract.
...
`export const router` is a plain object of `os.<procedure>.handler(...)` plus `plugins`. Handlers take `HaloContext` from `upgrade`; there are no `createHaloContext` / `createRouter` factories. `PluginService` keeps `router` as the mounted plugin map, fills it in `list()`, and exposes `route`. `HaloRPCHandler` serves that host `router` and maps nested `/plugins/...` calls onto `router.plugins`. Do not wrap the host object in `os.router()`; that hides extra keys behind the contract. Do not replace `PluginService.router`; `list()` deletes keys and assigns into the same record.
