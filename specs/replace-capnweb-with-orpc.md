...
Replace Cap'n Web with oRPC v2 over the same Electron MessagePort handshake. Main services live in oRPC initial context (`HaloContext`), provided at `RPCHandler.upgrade`. Procedures are a contract in `src/shared` and an implementer in main. Live session objects become `sessionId` plus procedures. The registry keeps one Pi session per id for the life of the RPC connection; route changes attach and detach the events iterator. Callbacks become `AsyncIteratorObject` streams. Plugin servers export an oRPC router; `PluginService.list()` mounts those routers onto `PluginService.router` and middleware injects `PluginServerContext`. The host `router.plugins` handler is `os.handler((request) => request.context.plugins.route(request))`. `PluginService.route` takes that same `ProcedureHandlerOptions` object. Nested `client.plugins[id].method()` sends `/plugins/{id}/method`; oRPC's matcher only indexes `/plugins`, so `pluginHandlerOptions` rewrites the URL to `/plugins` and puts the original path back on the handler options.
...
Assumption: oRPC lazy routers are for code splitting (`() => import(...)`) and do not receive request context. Plugin RPCs are dynamic, so Halo does not use lazy for them. Host procedures are a module-level `router` of `os.<procedure>.handler(...)` that read `HaloContext` from `upgrade`. `plugins` on that object is a handler that passes its `ProcedureHandlerOptions` to `context.plugins.route`. oRPC's matcher is exact-path and indexes only `/plugins`, so stock `RPCHandler` uses `pluginHandlerOptions`: a routing interceptor rewrites `/plugins/{id}/...` to `/plugins`, and a later interceptor restores the URL path onto `request.path`. `PluginService.route` then `getRouter` / `call`s the mounted procedure. Do not wrap the host object in `os.router()`; that hides extra keys behind the contract. Do not subclass `RPCHandler` or `RPCHandlerCodec`.
...
`export const router` is a plain object of `os.<procedure>.handler(...)` plus `plugins`. Handlers take `HaloContext` from `upgrade`; there are no `createHaloContext` / `createRouter` factories. `PluginService` keeps `router` as the mounted plugin map, fills it in `list()`, and exposes `route(request)` with oRPC's `ProcedureHandlerOptions`. Main constructs stock `RPCHandler(router, pluginHandlerOptions)`. Do not wrap the host object in `os.router()`; that hides extra keys behind the contract. Do not replace `PluginService.router`; `list()` deletes keys and assigns into the same record.
