...
Assumption: oRPC's RPCMatcher indexes procedures when `RPCHandler` is constructed. Host procedures are a module-level `router` of `os.<procedure>.handler(...)` that read `HaloContext` from `upgrade`. `plugins` stays on that object as `os.lazy(() => ({ default: context.plugins.router }))`. oRPC lazy loaders do not receive context, so `bindHaloContext` (a routing interceptor) stores `HaloContext` in `AsyncLocalStorage` for the load. The matcher indexes that map on the first `/plugins/...` call. Halo calls `listPlugins` before plugin RPCs, so that load sees the filled map. Lazy loads once per handler; a new MessagePort (window reload) builds a new handler. If a call lands before `listPlugins`, the procedure is missing and oRPC returns its normal not-found error. Do not wrap the host object in `os.router()`; that hides extra keys behind the contract.
...
`export const router` is a plain object of `os.<procedure>.handler(...)` plus `plugins`. Handlers take `HaloContext` from `upgrade`; there are no `createHaloContext` / `createRouter` factories. `PluginService` keeps `router` as the mounted plugin map and fills it in `list()`. Pass that same `router` to `RPCHandler` with `routingInterceptors: [bindHaloContext]`. Do not wrap the host object in `os.router()`; that hides extra keys behind the contract. Do not replace `PluginService.router`; `list()` deletes keys and assigns into the same record.
