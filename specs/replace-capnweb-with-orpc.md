...
Replace Cap'n Web with oRPC v2 over the same Electron MessagePort handshake. Main services live in oRPC initial context (`HaloContext`), provided at `RPCHandler.upgrade`. Procedures are a contract in `src/shared` and an implementer in main. Live session objects become `sessionId` plus procedures. The registry keeps one Pi session per id for the life of the RPC connection; route changes attach and detach the events iterator. Callbacks become `AsyncIteratorObject` streams. Plugin servers export an oRPC router; `PluginService.list()` mounts those routers onto its lazy `orpcRouter` and middleware injects `PluginServerContext`.
...
Assumption: oRPC's RPCMatcher indexes procedures when `RPCHandler` is constructed. `PluginService` owns the mounted plugin map and exposes `orpcRouter` as `os.lazy(() => ({ default: this.mounted }))`. Host procedures are a module-level `router` of `os.<procedure>.handler(...)` that read `HaloContext` from `upgrade`. The handler is `{ ...router, plugins: pluginService.orpcRouter }` with the same `pluginService` on context. The matcher cannot read context, so it indexes the lazy map on the first `/plugins/...` call. Halo calls `listPlugins` before plugin RPCs, so that load sees the filled map. Lazy loads once per handler; a new MessagePort (window reload) builds a new handler. If a call lands before `listPlugins`, the procedure is missing and oRPC returns its normal not-found error. Do not wrap the host object in `os.router()`; that hides extra keys behind the contract.
...
`export const router` is a plain object of `os.<procedure>.handler(...)`. Handlers take `HaloContext` from `upgrade`; there are no `createHaloContext` / `createRouter` factories. `PluginService` keeps a private `mounted` map, fills it in `list()`, and exposes `orpcRouter`. Pass `{ ...router, plugins: pluginService.orpcRouter }` to `RPCHandler` and the same `pluginService` on context. Do not wrap that object in `os.router()`; that hides extra keys behind the contract. Do not replace the `mounted` object; `list()` deletes keys and assigns into the same record.
