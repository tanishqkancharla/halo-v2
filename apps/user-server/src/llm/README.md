...
Server tests and Electron E2Es share `LLMDriver` from `@get-halo/server/testing`. The driver hosts an OpenAI-compatible HTTP endpoint on a random loopback port. Server tests inject `createOpenAILLMApi(llm.configuration)`; Electron E2Es pass the same configuration to the independent user server through its launch environment. Both use Pi's real HTTP inference client.
